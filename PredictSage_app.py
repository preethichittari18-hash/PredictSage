...
source_type = st.sidebar.selectbox("Select Source:", [None, "File", "Server"], key="source_type", on_change=clear_variables)

if not source_type:
    load_landing()
    st.stop()

def update_upload(df):
    st.session_state.df = df
    st.session_state.active_tab = 0

def file_upload(main_container):
    file = main_container.file_uploader("Please upload a file:", ['csv', 'xls', 'xlsx', 'txt', 'json', 'db', 'sqlite'], key="file_uploader")
    ...
    if file_type == "excel":
        ...
        try:
            ...
            excel_sheet = main_container.selectbox("Select a sheet:", sheets, key="excel_sheet")
    ...
    if file_type == "sqlite":
        ...
        selected_table = main_container.selectbox("Select a table:", tables, key="sqlite_table")
        file.seek(0)
        try:
        ...
        has_headers = main_container.selectbox("Does the file have column headers?", [True, False], index=0, key="has_headers")
        try:
    ...
    st.button("Connect", on_click=update_upload, args=[df])

def server_upload(main_container):
    db_type = main_container.selectbox("Database type:", ["PostgreSQL", "MySQL", "SQL Server"], key="db_type")
    user = main_container.text_input("Username", key="db_user")
    password = main_container.text_input("Password", type="password", key="db_password")
    host = main_container.text_input("Host", value="localhost", key="db_host")
    port = main_container.text_input("Port", value="5432", key="db_port")
    database = main_container.text_input("Database name", key="db_name")
    try:
    ...
    selected_table = main_container.selectbox("Select a table:", tables, key="db_table")
    try:
...
with col3:
    st.markdown(f"**{st.session_state.tab_names[st.session_state.active_tab]}**", unsafe_allow_html=True)
with col4:
    if st.session_state.active_tab != -1 and st.session_state.active_tab < len(st.session_state.tab_names) - 2:
        st.button("➡️ Next", on_click=next_tab, args=[len(st.session_state.tab_names)])
...
def update_tab_names():
    type = st.session_state.analysis_type
    if type == "Univariate":
        st.session_state.tab_names = ["Data Preview", "Time Series Plot", "Seasonal Decomposition", "Model Metrics", "Actual vs Predicted", "Forecasted Results", ""]
    else:
        st.session_state.tab_names = ["Data Preview", "Time Series Plot", "Seasonal Decomposition", "ACF/PACF", "Correlation Heatmap", "Model Metrics", "Actual vs Predicted", "Forecasted Results", ""]

analysis_type = st.sidebar.selectbox("Select Analysis Type", [None, "Univariate", "Multivariate"], key="analysis_type", on_change=update_tab_names)
...
if analysis_type:
    ...
    date_column = st.sidebar.selectbox("Select the date column", [None] + list(st.session_state.df.columns))
...
if date_column and analysis_type:
    if analysis_type == "Univariate" and metric_column:
        ...
        # Display content based on active tab
        with main_container:
            if st.session_state.active_tab not in [0, 4] and len(metric_columns) < 2:
                st.warning("You need to select 2 or more columns for full analysis.")
                st.stop()
